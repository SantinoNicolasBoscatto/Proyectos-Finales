import { desactivarActivarBotones } from "./mini_funciones/disableEnable.js";
import { ocultarMostrar } from "./mini_funciones/hideShow.js";
...
export default async function loadList(version,url,back,next, object, filtroTipo=null, filtroName=null){
    try{
        desactivarActivarBotones(true, d);
        ...
        for (const el of json.results) {
            ...
            let count = 0;
            for (const key in json.types) {
                types += `<img src="./App/Assets/img/types/${json.types[key].type.name}.png"/>`
                count = key;
            }
            let v= "";
            let ds;
            ...
            `<img class="pokeImage pixel info" src="${json.sprites.versions[version]["ultra-sun-ultra-moon"]
            .front_default}" />`: version !== "generation-i" && version !== "generation-ii"?
            `<img class="pokeImage pixel info" src="${json.sprites.versions[version][ver].front_default}"/>` :
            `<img class="pokeImage pixel info" src="${json.sprites.versions[version][ver].front_transparent}"/>`
            ; 
            ...
            `<div style="position: absolute; bottom: 1rem; left: .5rem; height: 25px; width: 25px;">
                ...
                border: 1px solid #111" data-shiny="${dataShiny}" data-sprite="${ds}" class="sh">
                <img src="./App/Assets/img/shiny.png" class="shiny" data-shiny="${dataShiny}" data-sprite="${ds}"/>
            </button>
            </div>` : "";
            let cries = version !== "generation-i" && version !== "generation-ii"? 
                        json.cries.latest : json.cries.legacy;
            $fragHTML += 
            `<div class="carta" data-id="${json.id}" style="background: linear-gradient(to bottom right, 
                var(--${json.types[0].type.name}), var(--${json.types[count].type.name})); 
                position: relative;"">
                ${number}
                ${pokeimg} 
                <h5 class="info">${name}</h5>
                <div class="flex-type info" style="margin-bottom: .8rem;">
                    ${types}
                </div>
                <div class="flex-type info" style="margin-top: auto;">
                ${v}
                </div>
        ...
        if(filtroTipo === null && filtroName===null)desactivarActivarBotones(false, d);
    }
    catch(err){
        console.log(err)
    }
