    ...
    public class PilotosController : ControllerBase
    {
        ...
        public async Task<ActionResult<LecturaPilotoDTO>> GetPorId(int id)
        {
            try
            {
                ...
                if (piloto is null) return NotFound("El Piloto no existe");
        ...
        {
            if (pilotoDTO.FotoPiloto == null) return BadRequest("Porfavor Cargue una foto");
        ...
        public async Task<ActionResult> Put([FromForm] CrearPilotoDTO pilotoDTO, int id)
        {
            try
            {
                if (!await pilotoService.Existe(id)) return NotFound("El piloto a actualizar no existe");
